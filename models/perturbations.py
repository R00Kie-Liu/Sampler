...
def sample_noise_with_gradients(noise, shape):
    ...
    if noise == _GUMBEL:
        sampler = Gumbel(0.0, 1.0)
        k = shape[-1]
        samples = sampler.sample(shape[:-1]) # add noise to score (nums, B,M) instead of the whole score * k (nums, B,M,K)
        # samples = samples.unsqueeze(-1).expand(-1,-1,-1,k)
        samples = samples.unsqueeze(-1).expand([-1]*len(shape[:-1]) + [k])
        gradients = 1 - torch.exp(-samples)
    elif noise == _NORMAL:
        sampler = Normal(0.0, 1.0)
        k = shape[-1]
